<script lang="ts">
    ...
    let careplanTasks_ = data.careplanTasks_ || {};
    let selectedPlan = Object.keys(careplanTasks_)[0] || '';

    $: planCodes = Object.keys(careplanTasks_);
    $: currentTasks = selectedPlan ? careplanTasks_[selectedPlan] : [];
    
    $: careplanData = currentTasks[0] || {};
    $: careplanCode = careplanData?.Action?.PlanCode ?? '';
    $: careplanName = careplanData?.Action?.PlanName ?? '';

    $: taskStatusCounts = getTaskCounts(currentTasks);
    $: hasData = Object.keys(careplanTasks_).length > 0;
