<script lang="ts">
	...
	import { createTimeSeriesConfig, type ProcessedChartData } from '$lib/components/task.history/task.history.svelte';
	...
	const initChart = () => {
		if (timeSeriesCanvas && chartData?.datasets?.length > 0) {
			...
			if (tsCtx) {
				...
				const config: any = createTimeSeriesConfig(chartData);
